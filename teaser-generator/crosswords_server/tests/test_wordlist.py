...
def test_apple_is_valid() -> None:
    ...
    assert twl.is_twl_word("apple") is True
    assert twl.is_guess_word("APPLE") is True
...
def test_all_enabled_target_tiers_have_words() -> None:
    """Each enabled target tier should provide candidate pools."""
    assert len(twl.get_words_by_length(4, "CORE")) > 0
    assert len(twl.get_words_by_length(4, "STANDARD")) > 0
    assert len(twl.get_words_by_length(4, "ADVANCED")) > 0
    assert len(twl.get_words_by_length(4, "CANON")) > 0
