/**
 * src/lib/localAutoPlaceAllWords.ts
 * -----------------------------------------------------------
 * Deterministic port of server placement_impl.py (auto_place_all_words).
 * Places 5 words on a 10x10 grid with real crossword constraints:
 * - matching letters at intersections only
 * - no side-adjacent letters
 * - clear end-caps
 * - at most one intersection per pair of words
 * Returns the placed layout in original word order: { text, orient: 'H'|'V', coords }
 */
