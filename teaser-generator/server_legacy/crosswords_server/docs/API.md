...
## Dictionary (EOWL-based)

The server uses the EOWL-based word list at `data/wordlist_EOWL_mod.txt` (one word per line, case-insensitive; A–Z only).
If the file is absent, the server temporarily **soft-allows** any A–Z words for lengths 4–6 and prints a console warning.

Set env `TWL_PATH=/absolute/path/to/wordlist.txt` to override location.
