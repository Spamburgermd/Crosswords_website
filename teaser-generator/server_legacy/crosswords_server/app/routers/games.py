...
from ..services.validate import validate_wordset
from ..services import twl
from ..services.dictionary_slots import DictionarySlot, is_slot_enabled
...
def _resolve_and_validate_slot(slot_value: str | None) -> str:
    ...
    Raises HTTPException 400 if slot A/B/C (disabled).
    """
    slot = (slot_value or "STANDARD").upper()
    try:
        enum_slot = DictionarySlot(slot)
    except ValueError:
        raise HTTPException(400, f"Invalid dictionary slot: {slot}. Use STANDARD, A, B, or C.")
    if not is_slot_enabled(enum_slot):
...
BOT_PLACEMENT_MAX_ATTEMPTS = 20


def _default_bot_words(slot: str = "STANDARD") -> List[str]:
    """
...
def _candidate_pool_for_length(length_value: int, slot: str = "STANDARD") -> List[str]:
    """
...
def create_game(
    ...
    if data is not None and hasattr(data, "dictionary_slot") and data.dictionary_slot is not None:
        slot_raw = data.dictionary_slot.value
...
def bot_join(
    ...
    slot = getattr(game, "dictionary_slot", None) or "STANDARD"
...
def make_guess(
    ...
    if target_signature_raw:
        resolved_entry = signature_to_entry.get(target_signature_raw)
        if not resolved_entry:
            raise HTTPException(400, detail=f"Unknown target_signature {target_signature_raw}.")
        resolved_signature = target_signature_raw
        resolved_index = resolved_entry["target_index"]
        target_index = resolved_index
    else:
        resolved_entry = index_to_entry.get(target_index)
        if not resolved_entry:
            raise HTTPException(400, detail=f"Unknown target_index {target_index}.")
        resolved_signature = resolved_entry["signature"]
    ...
    slot = getattr(game, "dictionary_slot", None) or "STANDARD"
    if not twl.is_twl_word(guess, slot):
...
def game_state(game_id: int, session: Session = Depends(get_session), me: User = Depends(require_user)):
    ...
    return GameStateOut(
        ...
        dictionary_slot=getattr(game, "dictionary_slot", None) or "STANDARD",
